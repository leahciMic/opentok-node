...
Client.prototype.createSession = function(opts, cb) {
  ...
  console.error('fetch: ' + this.c.apiUrl + this.c.endpoints.createSession, fetchOptions);
  ...
  fetcher
    .then(function(resp) {
      console.error('fetch response: ' + resp.status, resp.statusText, resp.type, resp.ok, resp.headers, resp.url);
